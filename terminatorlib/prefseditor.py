...
#83a598:#d3869b:#8ec07c:#ebdbb2'}
    ...
    def set_profile_values(self, profile):
        ...
        # Inactive terminal shading of background
        widget = guiget('inactive_bgcolor_offset')
        widget.set_value(float(self.config['inactive_bgcolor_offset']))
        widget = guiget('inactive_bgcolor_offset_value_label')
        widget.set_text('%d%%' % (int(float(self.config['inactive_bgcolor_offset'])*100)))
    ...
    def on_inactive_bgcolor_offset_value_changed(self, widget):
        """Inactive background color offset setting changed"""
        value = widget.get_value()  # This one is rounded according to the UI.
        if value > 1.0:
          value = 1.0
        self.config['inactive_bgcolor_offset'] = value
        self.config.save()
        guiget = self.builder.get_object
        label_widget = guiget('inactive_bgcolor_offset_value_label')
        label_widget.set_text('%d%%' % (int(value * 100)))
